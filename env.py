...
class Env_MAPPO(object):
    ...
    max_data_clip_bits = 10 * 1048576  # Maximum data size clip (bits)
    ...
    def _calculate_comm_compute_times_energy_multi( # Calculate communication, computation times and energy
        ...
        energy_compute_edge = self.uav_chip_coeff * (self.uav_cpu_frequency**3) * time_edge_compute # UAV computation energy
        return (
            time_transmission,
            time_edge_compute,
            time_local_compute,
    ...
    def _update_ugv_states(self): # Update UGV positions, data queues, and channel states
        for i in range(self.num_ugvs):
            ...
            self.ugv_positions[i, 0:2] = np.clip(self.ugv_positions[i, 0:2], 0, self.area_side_length) # Clip XY within area
            self.ugv_positions[i, 2] = np.clip(self.ugv_positions[i, 2], self.ugv_min_height, self.ugv_max_height) # Clip Z
    ...
    def step(self, actions): # Execute one environment step
        ...
        for uav_id in range(self.num_uavs): # Process actions for each UAV
            ...
            intended_fly_distance = fly_dist_ratio * self.uav_flight_speed * self.time_step_fly # Intended flight distance
            ...
            if can_service_this_ugv:
                t_tr, t_edge, t_local, energy_compute_edge, data_offloaded, data_local = (
                    self._calculate_comm_compute_times_energy_multi(
                        uav_id, target_ugv_id, uav_offloading_action_ratio, uav_pos_next_step  # Use next position for calculation
                    )
                )
                ...
                if available_after_flight < required_energy_comp_base: # If not enough battery for computation and base service
                    ...
                    if data_local > 1e-6 or data_offloaded > 1e-6:
                        delay_this_task = max(t_local, t_tr + t_edge) # Task delay is max of local and (transmission + edge)
                    else:
